"""Task 3 trainer — Adds support for exponential decay schedules."""
...
from .agent3 import DEVICE, DQNAgent3
...
# ============================================================================
# Utility functions & Decay Schedules (Task 3.1 & 3.2)
# ============================================================================
...
def epsilon_by_step(step: int, cfg: Config3) -> float:
    """Calculate epsilon using either linear or exponential decay."""
    if getattr(cfg, "decay_schedule", "linear") == "exponential":
        # Exponential decay
        decay_rate = math.log(cfg.epsilon_start / cfg.epsilon_end) / cfg.epsilon_decay_steps
        ep = cfg.epsilon_start * math.exp(-decay_rate * step)
        return max(cfg.epsilon_end, ep)
    else:
        # Standard Linear decay
        frac = min(1.0, step / max(1, cfg.epsilon_decay_steps))
        return cfg.epsilon_start + frac * (cfg.epsilon_end - cfg.epsilon_start)

def temperature_by_step(step: int, cfg: Config3) -> float:
    """Calculate temperature using either linear or exponential decay."""
    if getattr(cfg, "decay_schedule", "linear") == "exponential":
        # Exponential decay
        decay_rate = math.log(cfg.temperature_start / cfg.temperature_end) / cfg.temperature_decay_steps
        temp = cfg.temperature_start * math.exp(-decay_rate * step)
        return max(cfg.temperature_end, temp)
    else:
        # Standard Linear decay
...
class DQNTrainer3:
    """End-to-end training loop for Task 3."""

    def __init__(self, cfg: Config3, *,
                 device: torch.device = DEVICE,
                 verbose: bool = True):
        self.cfg           = cfg
        self.device        = device
        self.verbose       = verbose
        self.agent:        Optional[DQNAgent3] = None
        self.history:      List[Dict]          = []
        self.eval_history: List[Dict]          = []
        self.train_minutes: float              = 0.0
        self.state_shape:  Optional[Tuple]     = None

    def train(self) -> DQNAgent3:
        cfg = self.cfg
        set_seed(cfg.seed)
        ...
        if self.verbose:
            ...
            print(
                f"[DQNTrainer3]  device={self.device}  "
                ...
                f"exploration={cfg.exploration} ({cfg.decay_schedule})  "
                f"semantic_info=False"
            )
        ...
        for episode in range(1, cfg.episodes + 1):
            ...
            result.update({
                ...
                "epsilon":          result.get("explore_param", 0.0), # Backwards compatibility for plotting
            })
    ...
    def _evaluate_with_q(self, evaluator: Evaluator, difficulty: int,
        ...
        class _GreedyPolicy(Policy):
            name = "dqn3_greedy_inproc"

            def __init__(inner, agent: DQNAgent3, stack: int):
                inner.agent      = agent
        ...
        # Randomize evaluation seed to prevent overfitting
